...
func main() {
	a := app.NewWithID("techinjekton.dev.tools.healthchecker")
	...
	setScreen := func(t display.Screen) {
...
func unsupportedTutorial(t display.Screen) bool {
	return !t.SupportWeb && fyne.CurrentDevice().IsBrowser()
}

func makeNav(setTutorial func(tutorial display.Screen), loadPrevious bool) fyne.CanvasObject {
	a := fyne.CurrentApp()

	tree := &widget.Tree{
		ChildUIDs: func(uid string) []string {
			return display.ScreenIndex[uid]
		},
		IsBranch: func(uid string) bool {
			children, ok := display.ScreenIndex[uid]
		...
		OnSelected: func(uid string) {
			if t, ok := display.Screens[uid]; ok {
				if unsupportedTutorial(t) {
					return
				}
